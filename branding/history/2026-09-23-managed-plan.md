# Superseded managed-service plan

Historical implementation record only. Superseded by the profile-only fork scope on 2026-09-23. The active checklist is ../../IMPLEMENTATION_CHECKLIST.md. Commercial launch gates below no longer apply.
...
- Managed models are the default; subscribers may optionally bring an OpenRouter key. Personal-provider usage is billed separately by that provider. Keep custom agents, workflows, tools, and project functionality.
- Official downloads are public. A signed-in account and active paid subscription are required to use the workspace, start tasks, add personal keys, and run either managed or BYOK models. No free BYOK or offline entitlement bypass. Every enabled managed catalog model is included; favorites/defaults are preferences.
...
## Subscription-required BYOK (2026-09-23)

- [x] Lock the desktop workspace and all CLI work commands until subscription verification succeeds.
- [x] Enforce subscription access at the local server and before each managed/BYOK provider request; fail closed on outages, expiry and logout.
- [x] Add subscriber-only OpenRouter key setup with account-bound OS-encrypted storage, no key readback and no keys in project configuration; allow credential removal even after expiry.
- [x] Expose a separate “Your key” provider/model picker; allow explicit OpenRouter model IDs without switching funding sources on failure.
- [x] Add desktop and CLI BYOK controls and explain that provider usage is charged separately.
- [x] Update website/product copy and upstream-protected overlays for the new paid-access policy.
- [x] Verify paid/unpaid/expired/offline access, credential isolation, routing, streaming/cancellation and package typechecks/builds. Evidence: account/vault/bridge tests, local-server middleware tests before handlers run, real macOS Keychain tests, provider/picker tests, isolated browser interaction checks and successful desktop/CLI/site builds. See `branding/SUBSCRIPTION_BYOK.md`.
- [ ] Verify installed macOS/Windows BYOK flows and a real funded provider request before release (external release/provider prerequisites).
...
- [x] Launch the development desktop in an isolated onboarding profile and verify that the provider entry opens signed-out account controls through the accessibility tree, with no provider-key form before subscription activation.
...
- [x] Load the enabled managed catalog and account preferences; expose an optional paid OpenRouter BYOK connection. This replaces the earlier requirement to remove all customer key entry.
- [x] Wire desktop managed access through a private loopback bridge, with account controls and the enabled catalog. Add a distinct subscription-required BYOK route and picker; keep personal credentials out of the engine and renderer readback. Routing, streaming, cancellation and key isolation are tested.
- [x] Provide subscription-required CLI/TUI managed models and optional OpenRouter BYOK with secure key setup. Keep upstream auto-update, sharing and provider-auth defaults disabled. CLI help/build and provider integration tests pass; installed-release verification is separate.
...
## 5. Website and public installation

- [x] Implement the approved brand across 42 public marketing, model, comparison, documentation and trust pages, plus account, subscription and public downloads.
- [ ] Publish relevant branded documentation and accurate product copy.
- [x] Allow anonymous listing and artifact tickets while retaining signed catalogs, checksums, private storage and expiring links.
- [ ] Implement private-storage delivery and authenticated app-update access, including Range requests and expiry handling.
- [x] Generate download listings from verified release metadata; schema 2 adds CLI artifacts without changing schema 1 desktop identities.
- [x] Replace the earlier paid-download gate with public installation. Keep same-origin browser forms and JSON native responses; paid access is enforced for the workspace, key setup and managed/BYOK tasks.
...
- [ ] Complete both public download → app login → verified payment → actual task and model page → payment → download → actual task acceptance tests.
...
- 2026-09-23: Landing/SEO implementation is available locally in the private `platform` checkout; see `platform/LAUNCH.md`. Added full-catalog entitlement, favorites/default migration, explicit production billing/Auth configuration, public desktop/CLI release support, draft-preserving client payment checks, and consented aggregate website measurement. Domain, price/currency, actual model availability/costs, signed releases, company policies and production credentials remain launch gates.

- 2026-09-23: Implemented subscription-required workspace access plus optional OpenRouter BYOK locally. Added account-bound encrypted personal-key storage, hidden CLI entry, account-only native IPC, a separate provider route, fresh entitlement checks on local server operations and model requests, and fail-closed offline/expiry behavior. Updated public-site access/billing/privacy copy and upstream regression checks. See `branding/SUBSCRIPTION_BYOK.md`; this work has not been deployed or released.

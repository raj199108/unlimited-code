...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `unlimitcode-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "unlimitcode", "log"), join(app.getPath("userData"), "unlimitcode", "log")])]
}

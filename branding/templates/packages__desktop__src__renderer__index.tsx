...
if (import.meta.env.VITE_SENTRY_DSN) {
  Sentry.init({
    ...
    release: import.meta.env.VITE_SENTRY_RELEASE ?? `desktop@${product.version}`,
    initialScope: {
      tags: {
        platform: "desktop",
      },
...
const createPlatform = (windowState: DesktopWindowState): Platform => {
  ...
  return {
    platform: "desktop",
    managedAccount: window.api.managedAccount,
    os,
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: new URL("/unlimit-code-icon.png", window.location.href).href,
      })

...
import { verifyAccess } from "@unlimitcode/account/access-client"
...
export const disposeMiddleware: HttpMiddleware.HttpMiddleware = (effect) =>
  Effect.gen(function* () {
    ...
    const health = ["GET", "HEAD"].includes(incoming.method) && ["/api/health", "/global/health"].includes(pathname)
    if (incoming.method !== "OPTIONS" && !health) {
      const permitted = yield* Effect.tryPromise(() => verifyAccess()).pipe(
        Effect.as(true),
        Effect.orElseSucceed(() => false),
      )
      if (!permitted) return HttpServerResponse.empty({ status: 402, headers: { "cache-control": "no-store" } })
    }

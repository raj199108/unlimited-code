...
// Only test processes own this synthetic account; production has no test bypass.
const accountAccess = await startAccessServer(async () => ({
  status: "signed-in",
  accessUntil: new Date(Date.now() + 3600000).toISOString(),
}))
Object.assign(process.env, accountAccess.environment)
afterAll(() => accountAccess.close())

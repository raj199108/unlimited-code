  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "unlimit-code")
        draft.active = typeof active === "string" ? active : "unlimit-code"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .catch(() => setStore("active", "unlimit-code"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "unlimit-code")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes["unlimit-code"], store.mode)
    })

...
const APP_NAMES: Record<string, string> = {
  dev: "Unlimit Code Dev",
  beta: "Unlimit Code Beta",
  prod: "Unlimit Code",
}
const APP_IDS = product.appIds
...
const main = Effect.gen(function* () {
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Unlimit Code Dev")
  ...
  const receiveLinks = async (urls: string[]) => {
    for (const url of urls) {
      ...
      if (!parsed || parsed.protocol !== `${product.schemes[CHANNEL]}:`) continue
      if (parsed.hostname === "auth") {
        await account.callback(url).catch(() => logger.warn("account callback failed"))
        continue
      }
      emitDeepLinks([url])
    }
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith(`${product.schemes[CHANNEL]}://`))
    if (urls.length) {
      void receiveLinks(urls)
    }
    const win = getLastFocusedWindow()
    if (win) {
      win.show()
  ...
  if (app.isPackaged) app.setAsDefaultProtocolClient(product.schemes[CHANNEL])
  // Development launches need the entrypoint as well as the Electron executable.
  if (!app.isPackaged && developmentEntry)
    app.setAsDefaultProtocolClient(product.schemes[CHANNEL], process.execPath, [developmentEntry])
  void receiveLinks(process.argv)
  registerRendererProtocol()
  setDockIcon()

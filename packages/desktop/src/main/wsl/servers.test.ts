...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Unlimit Code update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Unlimit Code", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("ignores stale background Unlimit Code checks after removing a WSL server", async () => {
  persistedServers = []
...
test("ignores stale startup Unlimit Code checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Unlimit Code", async () => {
  persistedServers = []
...
test("does not check Unlimit Code in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const opencode: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),

---
title: دعم ACP
description: استخدم Unlimit Code في أي محرر متوافق مع ACP.
---

يدعم Unlimit Code بروتوكول [Agent Client Protocol](https://agentclientprotocol.com) أو (ACP)، مما يتيح لك استخدامه مباشرة في المحررات وبيئات التطوير المتكاملة (IDEs) المتوافقة.
...
لاستخدام Unlimit Code عبر ACP، اضبط محررك ليشغّل الأمر `opencode acp`.

يشغّل هذا الأمر Unlimit Code كعملية فرعية متوافقة مع ACP تتواصل مع محررك عبر JSON-RPC باستخدام stdio.
...
ثبّت Unlimit Code من [سجل ACP في Zed](https://zed.dev/docs/ai/external-agents#registry) عبر تشغيل `zed: acp registry` من لوحة الأوامر.

لاستخدام ملف Unlimit Code تنفيذي مخصص بدلاً من ذلك، أضفه إلى إعدادات [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
لفتحه، اختر الوكيل الجديد 'Unlimit Code' من محدد الوكلاء في AI Chat.
...
لاستخدام Unlimit Code كوكيل ACP في [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)، أضف التالي إلى إعدادات Neovim:
...
يضبط هذا الإعداد CodeCompanion لاستخدام Unlimit Code كوكيل ACP للدردشة.
...
يعمل Unlimit Code عبر ACP بالطريقة نفسها التي يعمل بها في terminal. جميع الميزات مدعومة:
...
- خوادم MCP المضبوطة في إعدادات Unlimit Code

---
title: المزوّدون
description: استخدام أي موفّر LLM في Unlimit Code.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Unlimit Code ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Unlimit Code.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Unlimit Code.
...
Unlimit Code Zen هي قائمة نماذج يوفّرها فريق Unlimit Code وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Unlimit Code. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Unlimit Code Zen.
:::
...
يعمل مثل أي مزوّد آخر في Unlimit Code واستخدامه اختياري بالكامل.

---

## Unlimit Code Go

Unlimit Code Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Unlimit Code، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Unlimit Code.
...
لاستخدام Amazon Bedrock مع Unlimit Code:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Unlimit Code غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم Unlimit Code نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Unlimit Code
...
لاستخدام Google Vertex AI مع Unlimit Code:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Unlimit Code تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Unlimit Code مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Unlimit Code. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Unlimit Code:
...
3. انسخ مفتاح API لاستخدامه في Unlimit Code.
...
6. **مهم**: قبل استخدام نماذج السحابة في Unlimit Code، يجب سحب معلومات النموذج محليا:
...
Unlimit Code Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Unlimit Code. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Unlimit Code Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Unlimit Code Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Unlimit Code.
...
تسمح حقول `limit` لـ Unlimit Code بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.

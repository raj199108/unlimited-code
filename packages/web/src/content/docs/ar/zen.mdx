---
title: Zen
description: قائمة منتقاة من النماذج يوفّرها Unlimit Code.
---
...
Unlimit Code Zen هي قائمة بالنماذج التي اختبرها فريق Unlimit Code وتحقّق منها.

يعمل Zen مثل أي مزوّد آخر في Unlimit Code. تسجّل الدخول إلى Unlimit Code Zen وتحصل على
مفتاح API. استخدامه **اختياري بالكامل** ولا تحتاج إليه لاستخدام Unlimit Code.
...
:::tip
اختبرنا مجموعة منتقاة من النماذج والمزوّدين الذين يعملون جيدا مع Unlimit Code.
:::
...
Unlimit Code Zen هي بوابة AI تتيح لك الوصول إلى هذه النماذج.

---

## كيف يعمل

يعمل Unlimit Code Zen مثل أي مزوّد آخر في Unlimit Code.

1. تسجّل الدخول إلى **<a href={console}>Unlimit Code Zen</a>**، وتضيف تفاصيل الفوترة، ثم تنسخ مفتاح API.
2. تشغّل الأمر `/connect` في واجهة TUI، وتختار Unlimit Code Zen، ثم تلصق مفتاح API.
...
يستخدم [معرّف النموذج](/docs/config/#models) في إعدادات Unlimit Code الصيغة `opencode/<model-id>`. على سبيل المثال، بالنسبة إلى GPT 5.5، ستستخدم `opencode/gpt-5.5` في إعداداتك.
...
استخدم مفتاح API الخاص بـ Unlimit Code Zen مع نقطة النهاية `https://opencode.ai/zen/v1/systemone`. يتحقق هذا المثال مما إذا كان طلب الدعم عاجلا:
...
قد تلاحظ [نماذج منخفضة التكلفة](/docs/config/#models)، مثل Haiku أو Nano أو Flash، في سجل الاستخدام. يستخدم Unlimit Code هذه النماذج لإنشاء عناوين الجلسات.
...
- MiMo-V2.6-Flash Free متاح على Unlimit Code لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- MiMo-V2.5 Free متاح على Unlimit Code لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Ling 3.0 Flash Fin Free متاح على Unlimit Code لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Nemotron 3 Ultra Free متاح على Unlimit Code لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Nemotron 3.5 Lightning Free متاح على Unlimit Code لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Big Pickle نموذج خفي ومتاح مجانا على Unlimit Code لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Muse Spark 1.3 Contributor Free متاح على Unlimit Code لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Jev 1.13 Free متاح على Unlimit Code لفترة محدودة.
...
أنشأنا Unlimit Code Zen من أجل:
...
4. **عدم فرض أي ارتباط حصري** عبر تمكينك من استخدامه مع أي عامل برمجة آخر. ومع إبقاء إمكانية استخدام أي مزوّد آخر مع Unlimit Code أيضا دائما.

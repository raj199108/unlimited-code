---
title: CLI
description: Unlimit Code CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Unlimit Code CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa Unlimit Code.
...
Pokrenite Unlimit Code terminalski korisnički interfejs.
...
Unlimit Code CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte Unlimit Code agentima.
...
Priključite terminal na već pokrenut Unlimit Code backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim Unlimit Code backend-om. Na primjer:
...
Unlimit Code pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se Unlimit Code pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Unlimit Code.
...
Pokrenite Unlimit Code u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti Unlimit Code server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `opencode`) |
...
Pokrenite Unlimit Code headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti Unlimit Code-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Upravljajte Unlimit Code sesijama.
...
Navedite sve Unlimit Code sesije.
...
Prikaži statistiku upotrebe tokena i troškova za vaše Unlimit Code sesije.
...
Uvezite podatke sesije iz JSON datoteke ili Unlimit Code dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili Unlimit Code dijeljenog URL-a.
...
Pokrenite Unlimit Code headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup Unlimit Code-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte Unlimit Code i uklonite sve povezane datoteke.
...
Ažurira Unlimit Code na najnoviju verziju ili određenu verziju.
...
Unlimit Code CLI prihvata sljedeće globalne zastavice.
...
Unlimit Code se može konfigurirati pomoću varijabli okruženja.

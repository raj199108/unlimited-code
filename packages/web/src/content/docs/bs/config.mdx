---
title: Konfiguracija
description: Korištenje Unlimit Code JSON konfiguracije.
---

Možete konfigurirati Unlimit Code koristeći JSON konfiguracijski fajl.

---

## Format

Unlimit Code podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Postavite svoju globalnu Unlimit Code konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Kada se Unlimit Code pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Unlimit Code server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Unlimit Code konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Unlimit Code pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Unlimit Code će se vratiti na `"build"` s upozorenjem.
...
Unlimit Code će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Unlimit Code **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Unlimit Code sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite Unlimit Code da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.

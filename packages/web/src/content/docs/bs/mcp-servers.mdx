...
Možete dodati vanjske alate u Unlimit Code koristeći _Model Context Protocol_, ili MCP. Unlimit Code podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Unlimit Code Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Unlimit Code automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Unlimit Code će:
...
Ako server zahtijeva autentifikaciju, Unlimit Code će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Unlimit Code sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Unlimit Code, zajedno s ugrađenim alatima. Možete njima upravljati kroz Unlimit Code konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Unlimit Code sa vašim Sentry nalogom.

---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Unlimit Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Unlimit Code koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Unlimit Code konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Unlimit Code
config.
...
Unlimit Code Zen je lista modela koje je obezbedio Unlimit Code tim koji su bili
testirano i potvrđeno da dobro radi sa Unlimit Code. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Unlimit Code Zen.
:::
...
Radi kao i svaki drugi provajder u Unlimit Code i potpuno je opcionalan za korištenje.

---

## Unlimit Code Go

Unlimit Code Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Unlimit Code tim i koji su testirani i verificirani da dobro rade s Unlimit Code-om.
...
Da biste koristili Amazon Bedrock s Unlimit Code:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Unlimit Code.
:::
...
Unlimit Code koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Unlimit Code
...
Za korištenje Google Vertex AI s Unlimit Code:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Unlimit Code razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Unlimit Code razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za Unlimit Code. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Unlimit Code:
...
3. Kopirajte API ključ za korištenje u Unlimit Code.
...
6. **Važno**: Prije upotrebe modela oblaka u Unlimit Code, morate lokalno povući informacije o modelu:
...
Unlimit Code Zen je lista testiranih i verifikovanih modela koju je obezbedio Unlimit Code tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Unlimit Code Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Unlimit Code Zen**.

```txt
   /connect
```

3. Unesite svoj Unlimit Code API ključ.
...
Polja `limit` omogućavaju Unlimit Code da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite Unlimit Code konfiguraciju i:

---
title: CLI
description: Unlimit Code CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The Unlimit Code CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with Unlimit Code programmatically.
...
Start the Unlimit Code terminal user interface.
...
The Unlimit Code CLI also has the following commands.

---

### agent

Manage agents for Unlimit Code.
...
Attach a terminal to an already running Unlimit Code backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote Unlimit Code backend. For example:
...
Unlimit Code is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When Unlimit Code starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in Unlimit Code.
...
Start a headless Unlimit Code server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
List all Unlimit Code sessions.
...
Delete an Unlimit Code session.
...
Show token usage and cost statistics for your Unlimit Code sessions.
...
Import session data from a JSON file or Unlimit Code share URL.

```bash
opencode import <file>
```

You can import from a local file or an Unlimit Code share URL.
...
Start a headless Unlimit Code server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access Unlimit Code through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Fetch and checkout a GitHub PR branch, then run Unlimit Code.
...
Uninstall Unlimit Code and remove all related files.
...
Unlimit Code can be configured using environment variables.

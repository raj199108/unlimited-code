...
description: Brug Unlimit Code i enhver ACP-kompatibel editor.
---

Unlimit Code understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge Unlimit Code via ACP, konfigurer din editor til at køre kommandoen `opencode acp`.

Kommandoen starter Unlimit Code som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
Installer Unlimit Code fra [Zeds ACP-register](https://zed.dev/docs/ai/external-agents#registry) ved at køre `zed: acp registry` i kommandopaletten.

Hvis du i stedet vil bruge en brugerdefineret Unlimit Code-eksekverbar fil, skal du føje den til din [Zed](https://zed.dev)-konfiguration (`~/.config/zed/settings.json`):
...
For at åbne den skal du bruge den nye 'Unlimit Code'-agent i AI Chat-agentvælgeren.
...
For at bruge Unlimit Code som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge Unlimit Code som ACP-agent til chat.
...
Unlimit Code fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din Unlimit Code-konfiguration

---
title: CLI
description: Unlimit Code CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Unlimit Code CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med Unlimit Code programmatisk.
...
Start Unlimit Code terminalbrugergrænsefladen.
...
Administrer agenter i Unlimit Code.
...
Tilslut en terminal til en allerede kørende Unlimit Code backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern Unlimit Code backend. For eksempel:
...
Unlimit Code drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når Unlimit Code starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i Unlimit Code.
...
Start en hovedløs Unlimit Code-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.
...
Administrator Unlimit Code sessionsholder.
...
Liste over alle Unlimit Code sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine Unlimit Code-sessioner.
...
Importer sessionsdata fra en JSON fil eller Unlimit Code del URL.
...
Start en hovedløs Unlimit Code-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til Unlimit Code via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller Unlimit Code og fjern alle relaterede filer.
...
Opdaterer Unlimit Code til den nyeste version eller en specifik version.
...
Unlimit Code kan konfigureres ved hjælp af miljøvariabler.

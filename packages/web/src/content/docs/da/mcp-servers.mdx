...
Du kan tilføje eksterne værktøjer til Unlimit Code ved hjælp af _Model Context Protocol_ eller MCP. Unlimit Code understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Unlimit Code Config](https://opencode.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Unlimit Code håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Unlimit Code:
...
Hvis serveren kræver godkendelse, vil Unlimit Code bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Unlimit Code opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Unlimit Code-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Unlimit Code til din Sentry-konto.

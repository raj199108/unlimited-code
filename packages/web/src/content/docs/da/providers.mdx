---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Unlimit Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Unlimit Code bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Unlimit Code-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i Unlimit Code-konfigurationen.
...
Unlimit Code Zen er en liste over modeller leveret af Unlimit Code-teamet, som er blevet testet og verificeret til at fungere godt med Unlimit Code. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Unlimit Code Zen.
:::
...
Det fungerer som alle andre udbydere i Unlimit Code og er helt valgfrit at bruge.

---

## Unlimit Code Go

Unlimit Code Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Unlimit Code-teamet, som er testet og verificeret til at fungere godt med Unlimit Code.
...
Sådan bruger du Amazon Bedrock med Unlimit Code:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Unlimit Code understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Unlimit Code bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Unlimit Code
...
Sådan bruger du GitHub Copilot-abonnementet med Unlimit Code:
...
Sådan bruger du Google Vertex AI med Unlimit Code:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver Unlimit Code-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Unlimit Code-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for Unlimit Code. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Unlimit Code:
...
3. Kopier API-nøglen til brug i Unlimit Code.
...
6. **Vigtigt**: Før du bruger skymodeller i Unlimit Code, skal du hente modelinformationen lokalt:
...
Unlimit Code Zen er en liste over testede og verificerede modeller leveret af Unlimit Code-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Unlimit Code Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Unlimit Code Zen**.

   ```txt
   /connect
   ```

3. Indtast Unlimit Code API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Unlimit Code:
...
`limit`-felterne lader Unlimit Code forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.

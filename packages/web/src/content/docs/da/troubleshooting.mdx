...
For at fejlfinde problemer med Unlimit Code, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Unlimit Code Desktop kører en lokal Unlimit Code-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
Unlimit Code kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Unlimit Code kan genopbygge den.

1. Afslut Unlimit Code Desktop helt.
2. Slet cache-mappen:
...
3. Start Unlimit Code Desktop på ny.

---

### Løs problemer med serverforbindelse

Unlimit Code Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
På Windows kræver Unlimit Code Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Unlimit Code.

---

### Meddelelser vises ikke

Unlimit Code Desktop viser kun systemvarsler når:

- varsler er aktiveret for Unlimit Code i dine OS-indstillinger, og
...
1. Afslut Unlimit Code Desktop.
2. Find og slet disse filer (de findes i Unlimit Code Desktop-appens datamappe):
...
Hvis du oplever problemer med Unlimit Code:

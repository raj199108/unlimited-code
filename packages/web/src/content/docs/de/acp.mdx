...
description: Verwenden Sie Unlimit Code in jedem ACP-kompatiblen Editor.
---

Unlimit Code unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um Unlimit Code über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `opencode acp`.

Der Befehl startet Unlimit Code als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Installieren Sie Unlimit Code aus der [Zed-ACP-Registry](https://zed.dev/docs/ai/external-agents#registry), indem Sie `zed: acp registry` in der Befehlspalette ausführen.

Wenn Sie stattdessen eine benutzerdefinierte Unlimit Code-Programmdatei verwenden möchten, fügen Sie sie Ihrer [Zed](https://zed.dev)-Konfiguration (`~/.config/zed/settings.json`) hinzu:
...
Um es zu öffnen, verwenden Sie den neuen Agenten „Unlimit Code“ in der AI Chat Agent Selector.
...
Um Unlimit Code als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass Unlimit Code als ACP-Agent für den Chat verwendet wird.
...
Unlimit Code funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer Unlimit Code-Konfiguration konfiguriert sind

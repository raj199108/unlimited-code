...
Es gibt zwei Arten von Agenten in Unlimit Code; Primäragenten und Subagenten.
...
Unlimit Code verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Wenn keine Temperatur angegeben ist, verwendet Unlimit Code modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Unlimit Code-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Unlimit Code-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Unlimit Code Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.

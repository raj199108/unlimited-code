---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Unlimit Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Unlimit Code verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Unlimit Code-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Unlimit Code anpassen
config.
...
Unlimit Code Zen ist eine Liste von Modellen, die vom Unlimit Code-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Unlimit Code funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Unlimit Code Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Unlimit Code aus und gehen Sie zu [opencode.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Unlimit Code und ist völlig optional.

---

## Unlimit Code Go

Unlimit Code Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Unlimit Code-Team getestet und verifiziert wurden, dass sie gut mit Unlimit Code funktionieren.
...
So verwenden Sie Amazon Bedrock mit Unlimit Code:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Unlimit Code ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Unlimit Code wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Unlimit Code ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Unlimit Code-Konfiguration hinzufügen.
...
um das Token im Unlimit Code-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Unlimit Code verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Unlimit Code
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Unlimit Code:
...
So verwenden Sie Google Vertex AI mit Unlimit Code:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Unlimit Code ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Unlimit Code konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede Unlimit Code-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Unlimit Code-Konversation als separate Sitzung aufgeführt.
...
Sie können Unlimit Code für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Unlimit Code über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Unlimit Code über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Unlimit Code konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit Unlimit Code:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Unlimit Code.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Unlimit Code verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Unlimit Code Zen ist eine Liste getesteter und verifizierter Modelle, die vom Unlimit Code-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Unlimit Code Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Unlimit Code Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Unlimit Code API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Unlimit Code-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Unlimit Code-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Unlimit Code:
...
Sie können Modelle auch über Ihre Unlimit Code-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Unlimit Code verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann Unlimit Code erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Unlimit Code-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Unlimit Code-Konfiguration übereinstimmt.

...
Wenn Unlimit Code Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Unlimit Code Desktop startet im Hintergrund einen lokalen Unlimit Code-Server (`opencode-cli`-Sidecar).
...
Unlimit Code kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Unlimit Code ihn neu aufbauen kann.

1. Beende Unlimit Code Desktop komplett.
...
3. Starte Unlimit Code Desktop neu.

---

### Server-Verbindungsprobleme beheben

Unlimit Code Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt Unlimit Code Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Unlimit Code-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Unlimit Code Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Unlimit Code in den OS-Einstellungen aktiviert sind, und
...
1. Beende Unlimit Code Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Unlimit Code Desktop):
...
Wenn du Probleme mit Unlimit Code hast:

---
title: Soporte ACP
description: Utilice Unlimit Code en cualquier editor compatible con ACP.
---

Unlimit Code admite el [Agent Client Protocol](https://agentclientprotocol.com) o (ACP), lo que le permite usarlo directamente en editores e IDE compatibles.
...
Para usar Unlimit Code a través de ACP, configure su editor para ejecutar el comando `opencode acp`.

El comando inicia Unlimit Code como un subproceso compatible con ACP que se comunica con su editor a través de JSON-RPC a través de stdio.
...
Instale Unlimit Code desde el [registro ACP de Zed](https://zed.dev/docs/ai/external-agents#registry) ejecutando `zed: acp registry` en la paleta de comandos.

Para usar un ejecutable personalizado de Unlimit Code, agréguelo a su configuración de [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
Para abrirlo, use el nuevo agente 'Unlimit Code' en el selector de agentes de AI Chat.
...
Para usar Unlimit Code como agente ACP en [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), agregue lo siguiente a su configuración de Neovim:
...
Esta configuración configura CodeCompanion para usar Unlimit Code como agente ACP para el chat.
...
Unlimit Code funciona igual a través de ACP que en la terminal. Todas las funciones son compatibles:
...
- Servidores MCP configurados en su configuración Unlimit Code

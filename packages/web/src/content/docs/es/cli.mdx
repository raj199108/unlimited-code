---
title: CLI
description: Opciones y comandos de la CLI de Unlimit Code.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de Unlimit Code de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con Unlimit Code mediante programación.
...
Inicie la interfaz de usuario del terminal Unlimit Code.
...
La CLI de Unlimit Code también tiene los siguientes comandos.

---

### agent

Administrar agentes para Unlimit Code.
...
Conecte una terminal a un servidor backend Unlimit Code que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto Unlimit Code. Por ejemplo:
...
Unlimit Code funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opencode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Cuando se inicia Unlimit Code, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en Unlimit Code.
...
Inicie un servidor Unlimit Code sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.
...
Administrar Unlimit Code sesiones.
...
Enumere todas las sesiones Unlimit Code.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones Unlimit Code.
...
Importe datos de sesión desde un archivo JSON o una URL compartida Unlimit Code.

```bash
opencode import <file>
```

Puede importar desde un archivo local o una URL compartida Unlimit Code.
...
Inicie un servidor Unlimit Code sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a Unlimit Code a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale Unlimit Code y elimine todos los archivos relacionados.
...
La CLI de Unlimit Code toma las siguientes banderas globales.
...
Unlimit Code se puede configurar mediante variables de entorno.

...
Puede agregar herramientas externas a Unlimit Code utilizando el _Model Context Protocol_ o MCP. Unlimit Code admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Unlimit Code Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Unlimit Code maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Unlimit Code:
...
Si el servidor requiere autenticación, Unlimit Code le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `opencode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Unlimit Code almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Unlimit Code, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Unlimit Code como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Unlimit Code a su cuenta Sentry.

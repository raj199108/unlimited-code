---
title: Proveedores
description: Usando cualquier proveedor LLM en Unlimit Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Unlimit Code usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Unlimit Code.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Unlimit Code
configuración.
...
Unlimit Code Zen es una lista de modelos proporcionados por el equipo Unlimit Code que han sido
probado y verificado para funcionar bien con Unlimit Code. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Unlimit Code Zen.
:::
...
Funciona como cualquier otro proveedor en Unlimit Code y su uso es completamente opcional.

---

## Unlimit Code Go

Unlimit Code Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Unlimit Code que han sido
probado y verificado para funcionar bien con Unlimit Code.
...
Para usar Amazon Bedrock con Unlimit Code:
...
:::info
El uso de su suscripción Claude Pro/Max en Unlimit Code no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Unlimit Code utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Unlimit Code
...
Para utilizar Google Vertex AI con Unlimit Code:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Unlimit Code como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Unlimit Code enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para Unlimit Code. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Unlimit Code:
...
3. Copie la clave API para usarla en Unlimit Code.
...
6. **Importante**: Antes de usar modelos en la nube en Unlimit Code, debe obtener la información del modelo localmente:
...
Unlimit Code Zen es una lista de modelos probados y verificados proporcionada por el equipo Unlimit Code. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Unlimit Code Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Unlimit Code Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Unlimit Code API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Unlimit Code:
...
Los campos `limit` le permiten a Unlimit Code comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.

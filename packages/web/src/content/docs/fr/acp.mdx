---
title: Support ACP
description: Utilisez Unlimit Code dans n’importe quel éditeur compatible ACP.
---

Unlimit Code prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser Unlimit Code via ACP, configurez votre éditeur pour exécuter la commande `opencode acp`.

La commande démarre Unlimit Code en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
Installez Unlimit Code depuis le [registre ACP de Zed](https://zed.dev/docs/ai/external-agents#registry) en exécutant `zed: acp registry` dans la palette de commandes.

Pour utiliser plutôt un exécutable Unlimit Code personnalisé, ajoutez-le à votre configuration [Zed](https://zed.dev) (`~/.config/zed/settings.json`) :
...
Pour l'ouvrir, utilisez le nouvel agent « Unlimit Code » dans le sélecteur d'agent AI Chat.
...
Pour utiliser Unlimit Code comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser Unlimit Code comme agent ACP pour le chat.
...
Unlimit Code fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration Unlimit Code

---
title: Configuration
description: Utilisation de la configuration JSON Unlimit Code.
---

Vous pouvez configurer Unlimit Code à l'aide d'un fichier de configuration JSON.

---

## Format

Unlimit Code prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
Placez votre configuration globale Unlimit Code dans `~/.config/opencode/opencode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Lorsque Unlimit Code démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Unlimit Code.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Unlimit Code via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Unlimit Code essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Unlimit Code reviendra à `"build"` avec un avertissement.
...
Unlimit Code téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent Unlimit Code avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre Unlimit Code à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.

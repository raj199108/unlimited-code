---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Unlimit Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Unlimit Code utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Unlimit Code.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Unlimit Code
configuration.
...
Unlimit Code Zen est une liste de modèles fournis par l'équipe Unlimit Code qui ont été
testé et vérifié pour fonctionner correctement avec Unlimit Code. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Unlimit Code Zen.
:::
...
Il fonctionne comme n’importe quel autre fournisseur dans Unlimit Code et son utilisation est totalement facultative.

---

## Unlimit Code Go

Unlimit Code Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Unlimit Code qui ont été
testé et vérifié pour fonctionner correctement avec Unlimit Code.
...
Pour utiliser Amazon Bedrock avec Unlimit Code :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Unlimit Code n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Pour utiliser Google Vertex AI avec Unlimit Code :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation Unlimit Code en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Unlimit Code répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour Unlimit Code. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Unlimit Code :
...
3. Copiez la clé API à utiliser dans Unlimit Code.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Unlimit Code, vous devez extraire les informations du modèle localement :
...
Unlimit Code Zen est une liste de modèles testés et vérifiés fournie par l'équipe Unlimit Code. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Unlimit Code Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Unlimit Code Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Unlimit Code API.
...
Les champs `limit` permettent à Unlimit Code de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.

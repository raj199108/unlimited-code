---
title: Web
description: En utilisant Unlimit Code dans votre navigateur.
---

Unlimit Code peut s'exécuter comme une application Web dans votre navigateur, offrant la même expérience puissante de codage d'IA sans avoir besoin d'un terminal.

![Unlimit Code Web - Nouvelle session](../../../assets/web/web-homepage-new-session.png)
...
Cela démarre un serveur local sur `127.0.0.1` avec un port disponible aléatoire et ouvre automatiquement Unlimit Code dans votre navigateur par défaut.
...
Par défaut, Unlimit Code sélectionne un port disponible. Vous pouvez spécifier un port :
...
Par défaut, le serveur se lie à `127.0.0.1` (localhost uniquement). Pour rendre Unlimit Code accessible sur votre réseau :

```bash
opencode web --hostname 0.0.0.0
```

Lorsque vous utilisez `0.0.0.0`, Unlimit Code affichera les adresses locales et réseau :
...
Une fois démarrée, l'interface web permet d'accéder à vos sessions Unlimit Code.
...
![Unlimit Code Web - Session active](../../../assets/web/web-homepage-active-session.png)

### Statut du serveur

Cliquez sur "Voir les serveurs" pour afficher les serveurs connectés et leur état.

![Unlimit Code Web - Voir Serveurs](../../../assets/web/web-homepage-see-servers.png)

---
title: Zen
description: Liste organisée de modèles fournis par Unlimit Code.
---
...
Unlimit Code Zen est une liste de modèles testés et vérifiés fournie par l'équipe Unlimit Code.

Zen fonctionne comme n'importe quel autre fournisseur dans Unlimit Code. Vous vous connectez à Unlimit Code Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Unlimit Code.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec Unlimit Code.
:::
...
Unlimit Code Zen est une passerelle AI qui vous donne accès à ces modèles.

---

## Comment ça marche

Unlimit Code Zen fonctionne comme n'importe quel autre fournisseur dans Unlimit Code.

1. Vous vous connectez à **<a href={console}>Unlimit Code Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez Unlimit Code Zen et collez votre clé API.
...
Le [model id](/docs/config/#models) dans votre configuration Unlimit Code utilise le format `opencode/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `opencode/gpt-5.5` dans votre configuration.
...
Utilisez votre clé API Unlimit Code Zen avec l’endpoint `https://opencode.ai/zen/v1/systemone`. Cet exemple vérifie si une demande d’assistance est urgente :
...
Vous remarquerez peut-être des [modèles à faible coût](/docs/config/#models), tels que Haiku, Nano ou Flash, dans votre historique d'utilisation. Unlimit Code utilise ces modèles pour générer les titres des sessions.
...
- MiMo-V2.6-Flash Free est disponible sur Unlimit Code pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- MiMo-V2.5 Free est disponible sur Unlimit Code pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Ling 3.0 Flash Fin Free est disponible sur Unlimit Code pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3 Ultra Free est disponible sur Unlimit Code pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3.5 Lightning Free est disponible sur Unlimit Code pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Big Pickle est un modèle stealth gratuit sur Unlimit Code pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Muse Spark 1.3 Contributor Free est disponible sur Unlimit Code pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Jev 1.13 Free est disponible sur Unlimit Code pour une durée limitée.
...
Nous avons créé Unlimit Code Zen pour :
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec Unlimit Code également.

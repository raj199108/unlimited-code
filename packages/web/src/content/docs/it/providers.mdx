---
title: Provider
description: Usa qualsiasi provider LLM in Unlimit Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Unlimit Code usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Unlimit Code.
...
configurazione di Unlimit Code.
...
Unlimit Code Zen e una lista di modelli forniti dal team Unlimit Code, testati e
verificati per funzionare bene con Unlimit Code. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Unlimit Code Zen.
:::
...
Funziona come qualsiasi altro provider in Unlimit Code ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Unlimit Code:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Unlimit Code non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Unlimit Code usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Unlimit Code
...
Per usare Google Vertex AI con Unlimit Code:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Unlimit Code come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Unlimit Code elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per Unlimit Code. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Unlimit Code:
...
3. Copia la chiave API da usare in Unlimit Code.
...
6. **Importante**: Prima di usare modelli cloud in Unlimit Code, devi scaricare le informazioni del modello localmente:
...
Unlimit Code Zen è una lista di modelli testati e verificati forniti dal team Unlimit Code. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Unlimit Code Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Unlimit Code Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Unlimit Code.
...
I campi `limit` permettono a Unlimit Code di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.

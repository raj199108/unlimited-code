...
Per diagnosticare problemi con Unlimit Code, inizia controllando i log e i dati locali che salva su disco.
...
Unlimit Code Desktop esegue in background un server locale di Unlimit Code (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
Unlimit Code puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Unlimit Code possa ricostruirla.

1. Chiudi completamente Unlimit Code Desktop.
...
3. Riavvia Unlimit Code Desktop.

---

### Risolvi problemi di connessione al server

Unlimit Code Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, Unlimit Code Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Unlimit Code.

---

### Notifiche non visualizzate

Unlimit Code Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Unlimit Code nelle impostazioni del sistema operativo, e
...
1. Chiudi Unlimit Code Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Unlimit Code Desktop):
...
Se riscontri problemi con Unlimit Code:

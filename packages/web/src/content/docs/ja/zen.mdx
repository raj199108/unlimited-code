---
title: Unlimit Code Zen
description: Unlimit Code によって提供されるモデルの厳選されたリスト。
---
...
Unlimit Code Zen は、Unlimit Code チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は Unlimit Code のほかのプロバイダーと同じように動作します。Unlimit Code Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Unlimit Code を使うために必須ではありません。
...
:::tip
Unlimit Code でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
Unlimit Code Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

Unlimit Code Zen は、Unlimit Code のほかのプロバイダーと同じように動作します。

1. **<a href={console}>Unlimit Code Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、Unlimit Code Zen を選んで API キーを貼り付けます。
...
Unlimit Code 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `opencode/gpt-5.5` を使用します。
...
Unlimit Code Zen API キーを `https://opencode.ai/zen/v1/systemone` エンドポイントで使用します。次の例では、サポートリクエストが緊急かどうかを確認します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。Unlimit Code はこれらのモデルをセッションタイトルの生成に使用します。
...
- MiMo-V2.6-Flash Free は期間限定で Unlimit Code で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で Unlimit Code で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Ling 3.0 Flash Fin Free は期間限定で Unlimit Code で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で Unlimit Code で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3.5 Lightning Free は期間限定で Unlimit Code で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で Unlimit Code で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Muse Spark 1.3 Contributor Free は期間限定で Unlimit Code で利用できます。チームはこの期間を活用してフィードバックを収集し、モデルを改善しています。
- Jev 1.13 Free は期間限定で Unlimit Code で利用できます。
...
私たちが Unlimit Code Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Unlimit Code では常にほかのプロバイダーも使えるようにすること。

...
OpenCode Zen は、Unlimit Code チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は Unlimit Code のほかのプロバイダーと同じように動作します。OpenCode Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Unlimit Code を使うために必須ではありません。
...
OpenCode Zen は、Unlimit Code のほかのプロバイダーと同じように動作します。

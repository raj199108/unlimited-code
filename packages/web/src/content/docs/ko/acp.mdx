...
Unlimit Code는 [Agent Client Protocol](https://agentclientprotocol.com)(ACP)을 지원하므로, ACP 호환 편집기와 IDE에서 Unlimit Code를 직접 사용할 수 있습니다.
...
ACP로 Unlimit Code를 사용하려면, 편집기에서 `opencode acp` 명령을 실행하도록 config를 설정하세요.

이 명령은 Unlimit Code를 ACP 호환 subprocess로 시작하며, stdio 기반 JSON-RPC를 통해 편집기와 통신합니다.
...
명령 팔레트에서 `zed: acp registry`를 실행하여 [Zed ACP 레지스트리](https://zed.dev/docs/ai/external-agents#registry)에서 Unlimit Code를 설치하세요.

대신 사용자 지정 Unlimit Code 실행 파일을 사용하려면 [Zed](https://zed.dev) 설정(`~/.config/zed/settings.json`)에 추가하세요.
...
[CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)에서 Unlimit Code를 ACP agent로 사용하려면 Neovim config에 다음을 추가하세요.
...
이 config는 chat에서 Unlimit Code를 ACP agent로 사용하도록 CodeCompanion을 설정합니다.
...
Unlimit Code는 ACP를 통해서도 터미널과 동일하게 동작합니다. 다음 기능을 모두 지원합니다.
...
- Unlimit Code config에 설정한 MCP 서버

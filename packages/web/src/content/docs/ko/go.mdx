...
Go는 Unlimit Code의 다른 제공자와 똑같이 작동합니다. OpenCode Go를 구독하고 API 키를 발급받으면 됩니다. 이는 **완전히 선택 사항**이며, Unlimit Code를 사용하기 위해 꼭 필요하지는 않습니다.
...
OpenCode Go는 Unlimit Code의 다른 제공자와 똑같이 작동합니다.
...
OpenCode Go는 [Unlimit Code](https://opencode.ai) 및 유사한 유형의 요청을 생성하는 다른 코딩 에이전트용으로 설계되었습니다.
...
Unlimit Code 외에도 다음 클라이언트가 OpenCode Go에서 올바르게 작동하는 것으로 검증되었습니다.

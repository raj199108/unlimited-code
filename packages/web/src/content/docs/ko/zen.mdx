...
OpenCode Zen은 Unlimit Code 팀이 제공하는, 테스트와 검증을 거친 모델 목록입니다.

Zen은 Unlimit Code의 다른 provider와 똑같이 작동합니다. OpenCode Zen에 로그인해 API 키를 받으면 됩니다. 이는 **완전히 선택 사항**이며, Unlimit Code를 사용하기 위해 반드시 사용할 필요는 없습니다.
...
OpenCode Zen은 Unlimit Code의 다른 provider와 똑같이 작동합니다.

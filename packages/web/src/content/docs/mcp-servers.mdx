...
You can add external tools to Unlimit Code using the _Model Context Protocol_, or MCP. Unlimit Code supports both local and remote servers.
...
You can define MCP servers in your [Unlimit Code Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Unlimit Code automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Unlimit Code will:
...
If the server requires authentication, Unlimit Code will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Unlimit Code will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in Unlimit Code, alongside built-in tools. So you can manage them through the Unlimit Code config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Unlimit Code to your Sentry account.

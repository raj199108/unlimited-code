...
Unlimit Code støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke Unlimit Code via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `opencode acp`.

Kommandoen starter Unlimit Code som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
Installer Unlimit Code fra [Zeds ACP-register](https://zed.dev/docs/ai/external-agents#registry) ved å kjøre `zed: acp registry` i kommandopaletten.

Hvis du i stedet vil bruke en egendefinert Unlimit Code-kjørbar fil, legger du den til i [Zed](https://zed.dev)-konfigurasjonen (`~/.config/zed/settings.json`):
...
For å åpne den, bruk den nye 'Unlimit Code'-agenten i AI Chat-agentvelgeren.
...
For å bruke Unlimit Code som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke Unlimit Code som ACP-agent for chat.
...
Unlimit Code fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i Unlimit Code-konfigurasjonen

...
description: Skriv dine egne programtillegg for å utvide Unlimit Code.
---

Plugins lar deg utvide Unlimit Code ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Unlimit Code.
...
Unlimit Code kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Unlimit Code SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Unlimit Code.
...
:::note
Hvis du bruker Unlimit Code-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Unlimit Code fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Unlimit Code:
...
`tool`-hjelperen lager et tilpasset verktøy som Unlimit Code kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Unlimit Code sammen med innebygde verktøy.

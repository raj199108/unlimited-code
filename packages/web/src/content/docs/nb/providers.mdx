---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Unlimit Code.
---

import config from "../../../../config.mjs"
export const console = config.console

Unlimit Code bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Unlimit Code-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Unlimit Code
konfig.
...
Unlimit Code Zen er en liste over modeller levert av Unlimit Code-teamet som har vært
testet og verifisert for å fungere godt med Unlimit Code. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Unlimit Code Zen.
:::
...
Det fungerer som alle andre leverandører i Unlimit Code og er helt valgfritt å bruke.

---

## Unlimit Code Go

Unlimit Code Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Unlimit Code-teamet som har vært
testet og verifisert for å fungere godt med Unlimit Code.
...
Slik bruker du Amazon Bedrock med Unlimit Code:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Unlimit Code:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Unlimit Code skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Unlimit Code-konfigurasjonen.
...
for å lagre token i Unlimit Code auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Unlimit Code bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Unlimit Code
...
Slik bruker du GitHub Copilot-abonnementet med Unlimit Code:
...
Slik bruker du Google Vertex AI med Unlimit Code:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Unlimit Code.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Unlimit Code, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Unlimit Code-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Unlimit Code-samtale oppført som en separat økt.
...
Du kan konfigurere Unlimit Code for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Unlimit Code for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Unlimit Code for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Unlimit Code. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Unlimit Code:
...
3. Kopier API-nøkkelen for bruk i Unlimit Code.
...
6. **Viktig**: Før du bruker skymodeller i Unlimit Code, må du hente modellinformasjonen lokalt:
...
Unlimit Code Zen er en liste over testede og verifiserte modeller levert av Unlimit Code-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Unlimit Code Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Unlimit Code Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Unlimit Code API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Unlimit Code-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Unlimit Code-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Unlimit Code:
...
Du kan også tilpasse modeller gjennom Unlimit Code-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Unlimit Code. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar Unlimit Code forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk Unlimit Code-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Unlimit Code-konfigurasjonen.

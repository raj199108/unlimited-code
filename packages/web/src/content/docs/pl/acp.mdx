---
title: ACP
description: Używanie Unlimit Code z protokołem Agent Client Protocol (ACP).
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Unlimit Code obsługuje [Agent Client Protocol](https://agentclientprotocol.com) (ACP), który umożliwia korzystanie z Unlimit Code bezpośrednio w edytorach i środowiskach IDE.
...
Aby używać Unlimit Code przez ACP, skonfiguruj swój edytor tak, aby uruchamiał polecenie `opencode acp`.

Uruchamia to Unlimit Code jako podproces zgodny z ACP, który komunikuje się z edytorem za pomocą JSON-RPC przez stdio.
...
Zainstaluj Unlimit Code z [rejestru ACP Zed](https://zed.dev/docs/ai/external-agents#registry), uruchamiając `zed: acp registry` w palecie poleceń.

Aby zamiast tego użyć niestandardowego pliku wykonywalnego Unlimit Code, dodaj go do konfiguracji [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
Następnie wybierz „Unlimit Code” w selektorze agentów AI Chat.
...
Aby używać Unlimit Code jako agenta ACP w [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), dodaj następującą konfigurację do Neovim:
...
Ta konfiguracja sprawia, że CodeCompanion używa Unlimit Code jako agenta ACP do czatu.
...
Unlimit Code działa przez ACP tak samo, jak w terminalu. Obsługiwane są wszystkie główne funkcje:
...
- Serwery MCP zdefiniowane w konfiguracji Unlimit Code

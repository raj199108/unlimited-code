---
title: Konfiguracja
description: Ustawienia Unlimit Code w pliku JSON.
---

Możesz dostosować Unlimit Code za pomocą pliku konfiguracyjnego JSON.

---

## Format

Unlimit Code obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Umieść swoją globalną konfigurację Unlimit Code w `~/.config/opencode/opencode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Kiedy Unlimit Code się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Unlimit Code.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Unlimit Code wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Unlimit Code powróci do `"build"` z ostrzeżeniem.
...
Unlimit Code może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Unlimit Code nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Unlimit Code **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Unlimit Code o niestandardowe narzędzia, hooki i integracje.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Unlimit Code tylko do określonych dostawców.

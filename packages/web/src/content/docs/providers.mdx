---
title: Providers
description: Using any LLM provider in Unlimit Code.
---

import config from "../../../config.mjs"
export const console = config.console

Unlimit Code uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Unlimit Code config.
...
You can customize the providers through the `provider` section in your Unlimit Code
config.
...
Unlimit Code Zen is a list of models provided by the Unlimit Code team that have been
tested and verified to work well with Unlimit Code. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Unlimit Code Zen.
:::
...
It works like any other provider in Unlimit Code and is completely optional to use.

---

## Unlimit Code Go

Unlimit Code Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Unlimit Code team that have been
tested and verified to work well with Unlimit Code.
...
To use Amazon Bedrock with Unlimit Code:
...
Unlimit Code. Anthropic explicitly prohibits this.

Previous versions of Unlimit Code came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Unlimit Code with zero setup:
...
Unlimit Code does not query Azure management APIs or discover deployments. Select a model whose catalog name matches your deployment, or configure its deployment name explicitly:
...
Assign your identity the inference role required by the deployment: **Cognitive Services OpenAI User** for Azure OpenAI models or **Cognitive Services User** for other Foundry models. Unlimit Code refreshes access tokens through the Azure CLI, including versions earlier than 2.54.0, so you only need to sign in again when the CLI session expires.
...
Unlimit Code supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Unlimit Code uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Unlimit Code. Sign in and approve.

   :::note
   Unlimit Code requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Unlimit Code surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Unlimit Code forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Unlimit Code is experimental. Features, configuration, and
behavior may change in future releases.
:::

Unlimit Code integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Unlimit Code uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Unlimit Code
...
When a `duo-workflow-*` model is selected, Unlimit Code will:
...
4. Route tool execution requests through Unlimit Code's permission-gated tool system
...
To use GMI Cloud with Unlimit Code:
...
To use Google Vertex AI with Unlimit Code:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Unlimit Code conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Unlimit Code conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for Unlimit Code. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Unlimit Code:
...
3. Copy the API key for use in Unlimit Code.
...
6. **Important**: Before using cloud models in Unlimit Code, you must pull the model information locally:
...
Unlimit Code Zen is a list of tested and verified models provided by the Unlimit Code team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Unlimit Code Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Unlimit Code Zen**.

   ```txt
   /connect
   ```

3. Enter your Unlimit Code API key.
...
5. Complete sign-in in the browser that opens. Unlimit Code captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
2. Select **SuperGrok Subscription**. Unlimit Code opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Unlimit Code. Unlimit Code polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Unlimit Code refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
The `limit` fields allow Unlimit Code to understand how much context you have left. Standard providers pull these from models.dev automatically.

...
Установите Unlimit Code из [реестра ACP Zed](https://zed.dev/docs/ai/external-agents#registry), выполнив `zed: acp registry` в палитре команд.

Чтобы вместо этого использовать собственный исполняемый файл Unlimit Code, добавьте его в конфигурацию [Zed](https://zed.dev) (`~/.config/zed/settings.json`):

...
Go, Unlimit Code'daki diğer sağlayıcılar gibi çalışır. OpenCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Unlimit Code'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenCode Go, Unlimit Code'daki diğer sağlayıcılar gibi çalışır.
...
OpenCode Go, [Unlimit Code](https://opencode.ai) ve benzer türde istekler üreten diğer kodlama aracıları için tasarlanmıştır.
...
Unlimit Code'un yanı sıra aşağıdaki istemcilerin OpenCode Go ile düzgün çalıştığı doğrulanmıştır.

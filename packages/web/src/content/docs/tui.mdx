---
title: TUI
description: Using the Unlimit Code terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Unlimit Code provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Unlimit Code starts the TUI for the current directory.
...
When using the Unlimit Code TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Unlimit Code. Allows you to select from available providers and add their API keys.
...
Exit Unlimit Code. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long Unlimit Code waits after the leader key. Defaults to `2000`.

---
title: Web
description: Using Unlimit Code in your browser.
---

Unlimit Code can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![Unlimit Code Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens Unlimit Code in your default browser.
...
By default, Unlimit Code picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make Unlimit Code accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, Unlimit Code will display both local and network addresses:
...
Once started, the web interface provides access to your Unlimit Code sessions.

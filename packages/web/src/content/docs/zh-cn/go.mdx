...
Go 的工作方式与 Unlimit Code 中的任何其他提供商（provider）一样。订阅 OpenCode Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 Unlimit Code 所必需的条件。
...
OpenCode Go 的工作方式与 Unlimit Code 中的其他提供商一样。
...
OpenCode Go 适用于 [Unlimit Code](https://opencode.ai) 以及其他会产生类似请求的编程 Agent。
...
除 Unlimit Code 外，以下客户端已通过验证，能够正常使用 OpenCode Go。但我们无法保证它们未来仍能正常使用。

...
Go 的運作方式與 Unlimit Code 中的任何其他供應商相同。您訂閱 OpenCode Go 並取得您的 API key。這是**完全可選的**，您不需要使用它也能使用 Unlimit Code。
...
OpenCode Go 的運作方式與 Unlimit Code 中的任何其他供應商相同。
...
OpenCode Go 適用於 [Unlimit Code](https://opencode.ai) 以及其他會產生類似請求的程式設計 Agent。
...
除了 Unlimit Code，以下用戶端已通過驗證，可正常使用 OpenCode Go。但我們無法保證它們未來仍能正常使用。
